...
fn test_health_print_no_panic() {
    let report = falcon::manage::health::HealthReport {
        ...
        dimensions: falcon::manage::health::HealthDimensions {
            code_quality: 70, security: 100, performance: 80,
            maintainability: 60, test_coverage: 50, dependency_health: 90,
        },
        top_priorities: vec![],
        stats: falcon::manage::health::ProjectStats {
            dart_files: 10, total_lines: 1000, total_issues: 50,
            error_count: 2, warning_count: 48, unused_files: 1, generated_files: 0,
        },
    };
...
fn test_dep_analysis_simple_pubspec() {
    let tmp = tempfile::tempdir().unwrap();
    std::fs::write(tmp.path().join("pubspec.yaml"), r#"
name: test_app
dependencies:
  http: ^1.0.0
  flutter:
...
"#).unwrap();
    ...
    assert!(report.direct_deps.iter().find(|d| d.name == "http").unwrap().is_used);
}
...
"#).unwrap();
    ...
    let report = falcon::manage::deps::analyze_dependencies(tmp.path()).unwrap();
    assert!(report.issues.iter().any(|i| matches!(i.issue_type, falcon::manage::deps::DepIssueType::GitDependency)));
}
...
fn test_arch_flat_project() {
    ...
    assert!(report.suggestions.iter().any(|s| s.contains("Clean Architecture") || s.contains("Feature-First")));
}

#[test]
fn test_arch_clean_architecture_detected() {
    let tmp = tempfile::tempdir().unwrap();
    for dir in &["lib/domain/entities", "lib/data/repos", "lib/presentation/pages"] {
        std::fs::create_dir_all(tmp.path().join(dir)).unwrap();
    }
    std::fs::write(tmp.path().join("lib/domain/entities/user.dart"), "class User {}\n").unwrap();
    std::fs::write(tmp.path().join("lib/data/repos/user_repo.dart"), "class UserRepo {}\n").unwrap();
    std::fs::write(tmp.path().join("lib/presentation/pages/home.dart"), "class HomePage {}\n").unwrap();
...
fn test_arch_hotspot_detection() {
    ...
    let report = falcon::manage::architect::analyze_architecture(tmp.path()).unwrap();
    assert!(!report.complexity_hotspots.is_empty(), "Should detect files >500 lines");
}
