...
fn test_cross_project_record_and_load() {
    ...
    std::fs::write(project.path().join("falcon.yaml"), "metrics:\n  cyclomatic_complexity: 20\n").unwrap();

    let profile = falcon::ai_score::cross_project::record_project(tmp.path(), project.path()).unwrap();
...
fn test_cross_project_deduplication() {
    ...
    let db = falcon::ai_score::cross_project::load_learning_db(tmp.path()).unwrap();
    assert_eq!(db.profiles.len(), 1, "Duplicate recordings should replace, not append");
}
...
fn test_cross_project_insights() {
    let db = falcon::ai_score::cross_project::LearningDatabase {
        ...
        architecture_distribution: [("Clean Architecture".to_string(), 1)].into_iter().collect(),
...
fn test_predict_print_no_panic() {
    let predictions = vec![
        falcon::ai_score::regression_predict::RiskPrediction {
...
fn test_discover_rules_finds_bang_operator() {
    ...
    let code: String = (0..10).map(|i| format!("final x{} = obj!.value;\n", i)).collect();
...
fn test_fix_tracking_record_and_load() {
    ...
    falcon::ai_score::fix_tracking::record_fix(
        tmp.path(), "avoid-dynamic", "lib/main.dart",
        falcon::ai_score::fix_tracking::FixOutcome::Accepted,
    ).unwrap();

    falcon::ai_score::fix_tracking::record_fix(
        tmp.path(), "avoid-dynamic", "lib/app.dart",
...
fn test_enterprise_default_policies() {
    ...
    assert!(policies.policies.iter().any(|p| p.name == "production-readiness"));
}
...
fn test_enterprise_audit_log() {
    ...
    falcon::platform::enterprise::record_audit(
        tmp.path(), "admin", "init", "policies", "created defaults"
    ).unwrap();
...
fn test_marketplace_search() {
    ...
    assert!(listings.iter().any(|l| l.name.contains("security") || l.description.to_lowercase().contains("security")));
}
...
fn test_partner_list() {
    ...
    assert!(partners.iter().any(|p| p.status == falcon::platform::partner::PartnerStatus::Certified));
}
