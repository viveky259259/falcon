...
"#);
    ...
    let report = falcon::asset_audit::audit_assets(dir.path()).unwrap();
    let has_oversized = report
        .issues
        .iter()
        .any(|i| i.category == "Oversized");
...
"#);
    ...
    let report = falcon::asset_audit::audit_assets(dir.path()).unwrap();
    let has_unused = report
        .issues
        .iter()
        .any(|i| i.category == "Unused");
...
fn asset_audit_html_report_is_generated() {
    let dir = TempDir::new().unwrap();
    write(&dir, "pubspec.yaml", "flutter:\n  assets:\n    - assets/img.png\n");
...
"#);
    ...
    assert!(has_color_issue, "Should detect hardcoded Color(0xFF...) usage");
}

#[test]
fn theme_audit_detects_hardcoded_font_size() {
    let dir = TempDir::new().unwrap();
    write(&dir, "lib/text_widget.dart", r#"
Text(
  'Hello',
...
fn l10n_coverage_detects_missing_translation() {
    ...
    write(&dir, "lib/l10n/app_en.arb", r#"
{
  "@@locale": "en",
  "hello": "Hello",
  "goodbye": "Goodbye",
...
"#);
    ...
    write(&dir, "lib/l10n/app_es.arb", r#"
{
  "@@locale": "es",
  "hello": "Hola"
}
...
fn l10n_coverage_perfect_translation() {
    let dir = TempDir::new().unwrap();
    write(&dir, "lib/l10n/app_en.arb", r#"{"@@locale":"en","hello":"Hello","bye":"Bye"}"#);
    write(&dir, "lib/l10n/app_fr.arb", r#"{"@@locale":"fr","hello":"Bonjour","bye":"Au revoir"}"#);
...
fn l10n_coverage_detects_empty_values() {
    let dir = TempDir::new().unwrap();
    write(&dir, "lib/l10n/app_en.arb", r#"{"@@locale":"en","title":"Title","body":"Body"}"#);
    write(&dir, "lib/l10n/app_de.arb", r#"{"@@locale":"de","title":"","body":"Hauptteil"}"#);
...
fn deeplink_detects_missing_android_manifest() {
    ...
    let has_android_issue = report
        .issues
        .iter()
        .any(|i| i.platform == "Android");
...
fn deeplink_extracts_android_schemes() {
  ...
  <application>
    ...
    let report = falcon::deeplink::validate_deeplinks(dir.path()).unwrap();
    assert!(
        report.android_schemes.iter().any(|s| s.contains("example.com") || s == "https"),
...
fn deeplink_detects_http_scheme_warning() {
  ...
  <application>
    ...
    let report = falcon::deeplink::validate_deeplinks(dir.path()).unwrap();
    let has_http_warn = report
        .issues
        .iter()
        .any(|i| i.detail.to_lowercase().contains("http") || i.category.to_lowercase().contains("http"));
...
fn animation_audit_detects_missing_dispose() {
    let dir = TempDir::new().unwrap();
    write(&dir, "lib/my_widget.dart", r#"
...
fn animation_audit_detects_setstate_in_listener() {
    let dir = TempDir::new().unwrap();
    write(&dir, "lib/bad_widget.dart", r#"
...
"#);
    ...
    let has_issue = report
        .issues
        .iter()
        .any(|i| i.category.to_lowercase().contains("setstate") || i.category.to_lowercase().contains("listener"));
    assert!(has_issue, "Should detect setState inside animation listener");
}

#[test]
fn animation_audit_clean_code_scores_high() {
    let dir = TempDir::new().unwrap();
    write(&dir, "lib/good_widget.dart", r#"
...
fn golden_gen_skips_generated_files() {
    let dir = TempDir::new().unwrap();
    write(&dir, "lib/my_widget.g.dart", r#"
