...
pub fn analyze_maintenance(root: &Path) -> anyhow::Result<MaintenanceReport> {
    ...
    let unused_code = report.issues.iter().filter(|i| i.rule == "unused-code").count();
    if unused_code > 0 {
    ...
    let unused_files = report.issues.iter().filter(|i| i.rule == "unused-file").count();
    if unused_files > 0 {
    ...
    let unused_deps = report.issues.iter().filter(|i| i.rule == "unused-dependency").count();
    if unused_deps > 0 {
        tasks.push(CleanupTask {
            category: CleanupCategory::UnusedDeps,
            description: format!("{} dependencies in pubspec.yaml are not imported", unused_deps),
            files_affected: 1,
            auto_fixable: false,
    ...
    if !codegen.stale_files.is_empty() {
        tasks.push(CleanupTask {
            ...
            description: format!("{} generated files may be stale (source missing)", codegen.stale_files.len()),
    ...
    let long_fns = report.issues.iter().filter(|i| i.rule == "avoid-long-functions").count();
    if long_fns > 20 {
        tasks.push(CleanupTask {
            category: CleanupCategory::LongFunctions,
            description: format!("{} functions exceed length threshold — extract helpers", long_fns),
    ...
    let empty_catches = report.issues.iter().filter(|i| i.rule == "avoid-empty-catch").count();
    if empty_catches > 0 {
        tasks.push(CleanupTask {
            category: CleanupCategory::EmptyCatches,
            description: format!("{} empty catch blocks silently swallowing errors", empty_catches),
    ...
    let prints = report.issues.iter().filter(|i| i.rule == "avoid-print-in-production").count();
    if prints > 0 {
        tasks.push(CleanupTask {
            category: CleanupCategory::PrintStatements,
            description: format!("{} print() calls in production code — replace with logger", prints),
    ...
    let trailing = report.issues.iter().filter(|i| i.rule == "prefer-trailing-comma").count();
    if trailing > 50 {
        tasks.push(CleanupTask {
...
pub fn print_maintenance_report(report: &MaintenanceReport) {
    println!();
    println!("  {} Maintenance Advisor", "falcon manage".bright_cyan().bold());
    println!();

    let debt_color = if report.tech_debt_score >= 80 { format!("{}", report.tech_debt_score).green() }
        else if report.tech_debt_score >= 50 { format!("{}", report.tech_debt_score).yellow() }
        else { format!("{}", report.tech_debt_score).red() };
    println!("  Tech Debt Score: {}/100", debt_color.bold());
    println!("  Cleanup tasks:   {} ({} auto-fixable, {} manual)",
        report.cleanup_tasks.len(), report.auto_fixable, report.manual_review);
    println!("  Est. savings:    ~{} lines removable", report.estimated_savings_lines);

    if !report.cleanup_tasks.is_empty() {
        println!();
        for task in &report.cleanup_tasks {
            let icon = if task.auto_fixable { "⚡".green() } else { "·".dimmed() };
            println!(
                "  {} [{}] {} ({} affected)",
                icon, task.category.to_string().bright_white(),
                task.description, task.files_affected
            );
            println!("     Run: {}", task.command.bright_cyan());
        }
    } else {
        println!();
        println!("  {} Codebase is clean — no maintenance tasks needed!", "✓".green().bold());
    }
