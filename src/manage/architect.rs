...
pub fn analyze_architecture(root: &Path) -> anyhow::Result<ArchReport> {
        ...
        .filter(|e| !e.path().to_string_lossy().contains(".g.dart"))
    {
        ...
        let dir = if parts.len() > 1 { parts[..parts.len()-1].join("/") } else { ".".to_string() };
    ...
    for (dir, (files, lines)) in &dir_stats {
        ...
        module_map.push(ModuleInfo {
            path: dir.clone(), file_count: *files, line_count: *lines, layer,
        });
    }
    ...
    if detected_pattern == "Flat/Custom" {
        suggestions.push("Consider adopting Clean Architecture or Feature-First structure for scalability".to_string());
        suggestions.push("Run: falcon refactor-sim --scenario clean-architecture".to_string());
    }
    if complexity_hotspots.len() > 5 {
        suggestions.push(format!("{} files exceed 500 lines — break them into focused modules", complexity_hotspots.len()));
    }
    if violations.len() > 10 {
        suggestions.push("Many architecture violations — enforce with falcon check-layers in CI".to_string());
    }

    Ok(ArchReport {
        detected_pattern, layer_compliance, violations,
        suggestions, module_map, complexity_hotspots,
    })
}

fn check_arch_violations(pattern: &str, file: &str, source: &str, violations: &mut Vec<ArchViolation>) {
    match pattern {
        "Clean Architecture" => {
            if file.contains("/domain/") && source.contains("import") {
                for line in source.lines() {
                    let trimmed = line.trim();
                    if trimmed.starts_with("import") && (trimmed.contains("/data/") || trimmed.contains("/presentation/")) {
            ...
            if file.contains("/data/") && source.lines().any(|l| l.trim().starts_with("import") && l.contains("/presentation/")) {
                violations.push(ArchViolation {
                    file: file.to_string(),
                    violation_type: "layer-violation".to_string(),
                    message: "Data layer imports from presentation — data should not know about UI".to_string(),
                });
            }
        }
        "Feature-First" => {
            if file.contains("/features/") {
                ...
                if parts.len() > 1 {
                    ...
                    for line in source.lines() {
                        let trimmed = line.trim();
                        if trimmed.starts_with("import") && trimmed.contains("/features/") && !trimmed.contains(feature) {
                            violations.push(ArchViolation {
                                file: file.to_string(),
...
fn classify_layer(pattern: &str, dir: &str) -> String {
    match pattern {
        "Clean Architecture" => {
            if dir.contains("domain") { "Domain" }
            else if dir.contains("data") { "Data" }
            else if dir.contains("presentation") { "Presentation" }
            else if dir.contains("core") { "Core" }
            else { "Other" }
        }
        "Feature-First" => {
            if dir.contains("features") { "Feature" }
            else if dir.contains("core") { "Core" }
            else if dir.contains("shared") { "Shared" }
            else { "Other" }
        }
        _ => "Unclassified",
    }.to_string()
...
pub fn print_arch_report(report: &ArchReport) {
    println!();
    println!("  {} Architecture Governor", "falcon manage".bright_cyan().bold());
    println!();

    println!("  Pattern:         {}", report.detected_pattern.bright_white().bold());
    ...
    if !report.violations.is_empty() {
        ...
        for v in report.violations.iter().take(10) {
            println!("    {} {} — {}", "✗".red(), v.file.bright_white(), v.message.dimmed());
        }
    ...
    if !report.complexity_hotspots.is_empty() {
        ...
        for h in report.complexity_hotspots.iter().take(5) {
            println!("    {} {} — {}", "⚠".yellow(), h.file.bright_white(), h.reason.dimmed());
        }
    }
