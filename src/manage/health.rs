...
pub fn generate_health_report(root: &Path) -> anyhow::Result<HealthReport> {
    ...
    let security = if vuln_findings.is_empty() { 100 }
        else { 100u32.saturating_sub(vuln_findings.len() as u32 * 15) };

    let perf_issues = perf_report.all_issues().len();
    let performance = 100u32.saturating_sub(perf_issues as u32 * 5);

    let long_fn = report.issues.iter().filter(|i| i.rule == "avoid-long-functions").count();
    let nested = report.issues.iter().filter(|i| i.rule == "avoid-nested-conditionals").count();
    let unused = report.issues.iter().filter(|i| i.rule.starts_with("unused")).count();
    ...
    let dep_issues = report.issues.iter().filter(|i| i.rule == "unused-dependency").count();
    ...
    if security < 80 {
        priorities.push(Priority {
            rank, category: "Security".to_string(),
    ...
    if ai_score.error_handling.score < 50 {
        priorities.push(Priority {
            rank, category: "Error Handling".to_string(),
            description: format!("Score {}/100 — empty catches, unawaited futures", ai_score.error_handling.score),
    ...
    if ai_score.resource_safety.score < 60 {
        priorities.push(Priority {
            rank, category: "Resource Safety".to_string(),
            description: format!("Score {}/100 — undisposed controllers", ai_score.resource_safety.score),
    ...
    if perf_issues > 10 {
        priorities.push(Priority {
            rank, category: "Performance".to_string(),
    ...
    if test_coverage < 30 {
        priorities.push(Priority {
            rank, category: "Test Coverage".to_string(),
            description: format!("{} test files for {} source files", test_files, report.file_count),
    ...
    Ok(HealthReport {
        ...
        dimensions: HealthDimensions {
            ...
            maintainability, test_coverage, dependency_health,
        },
        ...
        stats: ProjectStats {
            ...
            unused_files: report.issues.iter().filter(|i| i.rule == "unused-file").count(),
...
pub fn print_health_report(report: &HealthReport) {
    ...
    println!();
    println!("  Overall Health: {}/100 (Grade: {})", health_color, report.grade);
    println!();
    ...
    println!("    Files: {} ({} generated) | Lines: {} | Issues: {} ({} errors, {} warnings)",
        report.stats.dart_files, report.stats.generated_files,
        report.stats.total_lines, report.stats.total_issues,
...
fn print_bar(name: &str, score: u32, weight: u32) {
    ...
    let color = if score >= 80 { format!("{:>3}", score).green() }
        else if score >= 60 { format!("{:>3}", score).yellow() }
        else { format!("{:>3}", score).red() };
    println!("    {:<18} {}/100 {}{} ({}%)", name, color, bar.bright_cyan(), empty.dimmed(), weight);
}
