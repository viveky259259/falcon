...
type FalconScore = {
  overall: number;
  grade?: string;
  resource_safety?: FalconDimension;
  error_handling?: FalconDimension;
  type_safety?: FalconDimension;
  security?: FalconDimension;
  convention_match?: FalconDimension;
  complexity?: FalconDimension;
};
...
export function registerFalconChatParticipant(
  ...
  if (!chat?.createChatParticipant) {
    outputChannel.appendLine(
      "Falcon chat participant unavailable: VS Code Chat API is not present."
    );
    return undefined;
  }
...
function isScoreDropIntent(prompt: string, command: string | undefined): boolean {
...
function parseRuleIntent(
  ...
  const match = trimmed.match(
    /\bexplain(?:\s+rule)?\s+([a-z0-9][a-z0-9_-]*)\b/i
  );
  return match?.[1];
}

function firstRuleLikeToken(prompt: string): string | undefined {
  return prompt
    .split(/\s+/)
...
async function explainScoreDrop(
  context: vscode.ExtensionContext,
  ...
  const previous = context.workspaceState.get<ScoreHistory>(key);
...
function resolveScoreTarget(): vscode.Uri | undefined {
...
function runFalconScore(
  target: string,
  token: vscode.CancellationToken
): Promise<FalconScore> {
  return runFalconCommand(["score", target, "--format", "json"], token).then(
    (stdout) => JSON.parse(stdout) as FalconScore
  );
}

function runFalconCommand(
  args: string[],
  token: vscode.CancellationToken
): Promise<string> {
...
function scoreHistory(score: FalconScore): ScoreHistory {
  return {
...
function collectDimensionScores(score: FalconScore): Record<string, number> {
...
function collectFindings(score: FalconScore): string[] {
  return dimensionEntries(score).flatMap(([name, dimension]) =>
...
function dimensionEntries(
  score: FalconScore
): ReadonlyArray<readonly [string, FalconDimension | undefined]> {
  return [
...
function renderScoreDelta(
  ...
  previous: ScoreHistory | undefined
): string {
  const lines = [`Current Falcon score: **${current.overall}**.`];
  if (!previous) {
    lines.push(
      "No previous chat score is stored for this target yet. Ask again after the next change to see the delta."
    );
  ...
  const dimensionDeltas = Object.entries(current.dimensions)
    .map(([name, value]) => [name, value - (previous.dimensions[name] ?? value)] as const)
  ...
  if (dimensionDeltas.length > 0) {
    ...
    lines.push(
      ...dimensionDeltas.map(([name, value]) => {
...
function appendFindings(lines: string[], findings: string[]): string {
